install.packages(c("mixtools","tolerance","dlstats","ggplot","GGally","plotly","devtools",
                   "mixreg"))
...
ggplot(x[1:67,], aes(end, downloads, group=package, color=package)) + theme(legend.position = "none") + ggtitle("mixtools Monthly Downloads") +
  geom_line(lwd=1.7) + geom_point(aes(shape=package),size=3.5) + xlab("Month") + ylab("Downloads") + theme(text = element_text(size = 25))

for(i in 0:4) print(round(mean(x[(i*12+1):((i+1)*12),3]),0))

ggplot(x[68:134,], aes(end, downloads, group=package)) + theme(legend.position = "none") + ggtitle("tolerance Monthly Downloads") +
  geom_line(color="#00BFC4",lwd=1.7) + geom_point(aes(shape=package),color="#00BFC4",size=3.5) + xlab("Month") + ylab("Downloads") + theme(text = element_text(size = 25))
...
y <- read.table("http://astrostatistics.psu.edu/datasets/QSO_absorb.txt",nrows=104,skip=1)[,2]
...
mix.out.all=mixturegram(y,pmbs,method="pca",all.n=TRUE,id.con=NULL,main="Mixturegram (Quasars Data)")

all.ll=lapply(1:length(all.out),function(i) all.out[[i]]$loglik)
all.AIC=c(sum(dnorm(y,mean(y),sd(y),log=T))-2,sapply(1:length(all.out),function(i) all.ll[[i]]-(length(unlist(all.out[[i]][2:4]))-1)))
all.BIC=c(sum(dnorm(y,mean(y),sd(y),log=T))-log(300),sapply(1:length(all.out),function(i) all.ll[[i]]-(length(unlist(all.out[[i]][2:4]))-1)*log(300)/2 ))
all.ICL=c(all.BIC[1],sapply(1:length(all.out),function(i) all.BIC[i+1]-sum(all.out[[i]]$lambda*log(all.out[[i]]$lambda))))
all.CAIC=c(all.BIC[1]-1,sapply(1:length(all.out), function(i) all.BIC[i+1]-(length(unlist(all.out[[i]][2:4]))-1)/2) )
...
ggscatmat(DLBCLSample2,columns=1:3,color="Component") + theme(text = element_text(size = 20),legend.position = "none")

ggpairs(DLBCLSample2,columns=1:3,mapping=aes(alpha=.8)) + ggtitle("DLBCL Sample") + theme(text = element_text(size = 15))
ggpairs(DLBCLSample2,columns=1:3,mapping=aes(col=Component,pch=Component,alpha=.8)) + ggtitle("Estimated Two-Component Mixture of Trivariate Normals") + theme(text = element_text(size = 15))

fig <- plot_ly(DLBCLSample2, x = ~CD3, y = ~CD5, z = ~CD19, color = ~Component,colors = c('#F8766D', '#00BFC4'),size=10)
fig <- fig %>% add_markers()
fig
...
ggplot(data = aphids.df, aes(x, y)) + geom_point() + labs(title = "Aphids Data", x = "Number of Aphids in Batch", y = "Number of Infected Plants") + theme(text = element_text(size = 20))
...
ggplot(data = aphids.df1, aes(x, y, col = group, pch = group)) + geom_point() + labs(title = "Aphids Data (Parametric Mixture Fit)", 
  ...
  geom_abline(slope = out.em$beta[, 1][2], intercept = out.em$beta[, 1][1], col = 2) + geom_abline(slope = out.em$beta[, 2][2], intercept = out.em$beta[,2][1], col = 3) + 
  scale_color_manual("", values = c(2, 3), guide = F) + theme(text = element_text(size = 20), legend.position = "none")

set.seed(100)
spfit1 <- spregmix(n.inf~n.aphids, bw=.1, data=aphids, verb=TRUE)
...
ggplot(data = aphids.df2, aes(x, y, col = group, pch = group)) + geom_point() + labs(title = "Aphids Data (Semiparametric Mixture Fit; Symmetric Errors Assumption)", 
                                                                                     x = "Number of Aphids in Batch", y = "Number of Infected Plants") + 
  geom_abline(slope = spfit1$beta[, 1][2], intercept = spfit1$beta[, 1][1], col = 2) + geom_abline(slope = spfit1$beta[, 2][2], intercept = spfit1$beta[,2][1], col = 3) + 
  scale_color_manual("", values = c(2, 3), guide = F) + theme(text = element_text(size = 20), legend.position = "none")


set.seed(100)
spfit2 <- spregmix(n.inf~n.aphids, bw=.1, data=aphids, verb=TRUE, symm=FALSE)
...
ggplot(data=dens,aes(x=x,y=dens,color=Density)) + geom_line() + ylab("Density") + theme(text = element_text(size = 10)) + ggtitle("Kernel Density Estimates of Errors")
...
wells <- c(5.1, 2.4, 0.4, 0.5, 2.5, 0.1, 6.8,
...
ggplottol.hist(gam.out,x=wells,side="upper",x.lab = paste0("Vinyl Chloride Concentrations (",'&#956;',"g/L)"))
ggplottol.control(gam.out,x=wells,side="upper",x.lab = paste0("Vinyl Chloride Concentrations (",'&#956;',"g/L)"))
...
hospitals <- read.table("https://online.stat.psu.edu/stat462/sites/onlinecourses.science.psu.edu.stat462/files/data/infectionrisk/index.txt",header=TRUE)
...
tr.male <- npmvtol.region(x=as.matrix(ref.males[,-1]),alpha=0.05,P=0.95,depth.fn=Elliptical,type="semispace",adjust="ceiling",semi.order=list(lower=NULL,center=2:3,upper=1))

tr.male

male.df <- data.frame(C1=(ref.males[,2]<=tr.male[1,2]),
                      C2=((ref.males[,3]<=tr.male[2,2])&(ref.males[,3]>=tr.male[2,1])),
                      C3=((ref.males[,4]<=tr.male[3,2])&(ref.males[,4]>=tr.male[3,1])))

male.df <- data.frame(ref.males[,-1],C123=as.factor(male.df$C1&male.df$C2&male.df$C3))

ggplot(data = male.df, aes(UACR, Uric.Acid, col=C123, shape=C123)) + geom_rect(aes(xmin=0, xmax=tr.male[1,2], ymin=tr.male[2,1], ymax=tr.male[2,2]),fill = "gray80",alpha=.1, color="black", linetype=2) +
...
ggplot(data = male.df, aes(UACR, Serum.Creatinine, col=C123, shape=C123)) + geom_rect(aes(xmin=0, xmax=tr.male[1,2], ymin=tr.male[3,1], ymax=tr.male[3,2]),fill = "gray80",alpha=.1, color="black", linetype=2) +
  geom_point(size = 1.5) + theme(legend.position="none") +
...
ggplot(data = male.df, aes(Uric.Acid, Serum.Creatinine, col=C123, shape=C123)) + geom_rect(aes(xmin=tr.male[2,1], xmax=tr.male[2,2], ymin=tr.male[3,1], ymax=tr.male[3,2]),fill = "gray80",alpha=.1, color="black", linetype=2) +
  geom_point(size = 1.5) + theme(legend.position="none") +
  xlab("UA (mg/dL)") + ylab("SC (mg/dL)") + theme(text = element_text(size = 20)) +
  ggtitle("SC vs. UA Reference Regions (Males)") + xlim(1.75,15) + ylim(0,5)
