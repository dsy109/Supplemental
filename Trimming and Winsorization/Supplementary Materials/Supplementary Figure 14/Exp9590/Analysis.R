library(ggplot2)
setwd("/Users/kedaicheng/Desktop/Winsorization/Codes/Bivariate/Independent/Exp9590/Results")
#########################################
M <- 1000
P <- 0.95
alpha <- 0.10
...
###############################################
df.9590 <- data.frame(x.var , cov.org , cov.adj)
names(df.9590) <- c("Size","Unadjusted","Adjusted")

ggplot(data=df.9590)+
  geom_point(aes(x=x.var , y=df.9590$Unadjusted ,
                 col=rep("Unadj",length(x.var)),
                 shape=rep("Unadj",length(x.var))),cex=7)+
  geom_line(aes(x=x.var , y=df.9590$Unadjusted ,
                col=rep("Unadj",length(x.var))),lwd=3)+
  geom_point(aes(x=x.var , y=df.9590$Adjusted ,
                 col=rep("Adj",length(x.var)),
                 shape=rep("Adj",length(x.var))),cex=7) +
  geom_line(aes(x=x.var , y=df.9590$Adjusted ,
