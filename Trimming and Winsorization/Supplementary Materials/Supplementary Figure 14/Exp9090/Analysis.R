library(ggplot2)
setwd("/Users/kedaicheng/Desktop/Winsorization/Codes/Bivariate/Independent/Exp9090/Results")
#########################################
M <- 1000
P <- 0.90
alpha <- 0.10
...
###############################################
df.9090 <- data.frame(x.var , cov.org , cov.adj)
names(df.9090) <- c("Size","Unadjusted","Adjusted")

ggplot(data=df.9090)+
  geom_point(aes(x=x.var , y=df.9090$Unadjusted ,
                 col=rep("Unadj",length(x.var)),
                 shape=rep("Unadj",length(x.var))),cex=7)+
  geom_line(aes(x=x.var , y=df.9090$Unadjusted ,
                col=rep("Unadj",length(x.var))),lwd=3)+
  geom_point(aes(x=x.var , y=df.9090$Adjusted ,
                 col=rep("Adj",length(x.var)),
                 shape=rep("Adj",length(x.var))),cex=7) +
  geom_line(aes(x=x.var , y=df.9090$Adjusted ,
